...
class RockPaperScissorsGUI:
    ...
    def __init__(self, master):
        ...
        self.pokemon_image = None  # Prevent garbage collection
        
        # --- Critical Hit 시스템 ---
        self.critical_chance = 0.10  # 10% 확률
    ...
    def show_win_popup(self):
        ...
        try:
            ...
            self.pokemon_image = tk.PhotoImage(data=base64_data).zoom(2)

            image_label = tk.Label(popup, image=self.pokemon_image)
            image_label.pack(pady=10)

        except Exception as e:
            print(f"Pokemon API Error: {e}")
    ...
    def play_round(self, player_final_choice_korean):
        ...
        if outcome == 1:
            winner = 'player'
            self.player_score += 1  # 기본 승점 +1

            # --- Critical Hit 시스템 ---
            if random.random() < self.critical_chance:
                self.player_score += 1  # 추가 점수
